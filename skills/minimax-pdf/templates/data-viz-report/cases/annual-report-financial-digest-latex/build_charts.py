...
# Output directory: respect $BUILD_CHARTS_OUT_DIR if set, otherwise write next to this script
# under a `petrochina_2024_summary/` folder. The mavis-era default of
# /Users/minimax/.mavis/agents/main/workspace/petrochina_2024_summary was a
# hardcoded per-user path; in pi there is no equivalent agent workspace, so
# we use a path next to the script (or override via env var).
import os
_default = Path(__file__).resolve().parent / 'petrochina_2024_summary'
out = Path(os.environ.get('BUILD_CHARTS_OUT_DIR', str(_default)))
