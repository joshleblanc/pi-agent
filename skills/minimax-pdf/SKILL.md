...
`read_pdf_vision.py` additionally requires the matrix MCP to be loaded in the session
(vision tool shows up as `mcp__matrix__describe_images` or similar). The script does the
rendering and chunking; the agent invokes the MCP tool once per chunk and aggregates the
results. There is no daemon in pi — the MCP server runs in-process per session.
