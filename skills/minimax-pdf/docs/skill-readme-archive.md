...
scripts/
  ...
  read_pdf_vision.py          <- pages → matrix MCP describe_images via daemon [READ vision]
...
`scripts/read_pdf_vision.py` additionally requires the matrix MCP to be loaded
in the session (vision tool shows up as `mcp__matrix__describe_images` or
similar). The script does the chunking; the agent invokes the MCP tool once
per chunk and aggregates the results. There is no daemon in pi. See
`docs/vision-guide.md`.
