...
>
> - `<SKILL>` → the minimax-pdf skill folder (in pi: `~/.pi/agent/skills/minimax-pdf/`)
> - `<SKILL_XLSX>` → the minimax-xlsx skill folder (in pi: `~/.pi/agent/skills/minimax-xlsx/`)
...
## P4 — Multi-jurisdiction research with the `team` tool
...
STEP 1 — Plan structure (timeout-with-degrade is mandatory)
  Build a `team` plan with N parallel research workers + 1 synthesis
  worker. CRITICAL plan-level rules:
  - max_concurrency: N (one per jurisdiction)
  - max_consecutive_failures: 2
  - max_cycles: 10
  - Synthesis worker has depends_on: [all research workers], BUT its
...
STEP 2 — Per-worker prompt SHAPE (specific, not open-ended)
...
STEP 3 — Synthesis worker prompt
...
STEP 4 — Launch + monitor
  team { plan_file: "<TMP>/plan.yaml" }
  # Owner (you) does NOT poll — the engine pushes CycleReports.

STEP 5 — Mid-plan failure handling
  Worker crashed → engine retries automatically.
  Retry budget exhausted → CycleReport arrives → in your decision,
    set the failed task verdict=override_accept and let synthesis run
    on the remaining N-1 workers.
  Synthesis worker also missing data at deadline → already handled
    by STEP 1's partial-ship rule.

STEP 6 — Final verify (after synthesis worker delivers)
...
CONSTRAINTS:
  ...
  - Worker prompts MUST be specific. "全面研究" / "全面分析" = guaranteed stall.
  - Don't suppress stderr in any worker prompt — log to <TMP>/<worker>.log.
```
