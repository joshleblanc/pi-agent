...
def cache_dir() -> Path:
    """Return ~/.cache/pi/pdf-out/ — the spill destination for outputs
    ...
    The mavis-era path was ~/.cache/mavis/pdf-out. The pi equivalent is
    ~/.cache/pi/pdf-out. If you have leftover data under the old path,
    move it: `mv ~/.cache/mavis/pdf-out ~/.cache/pi/pdf-out`.
    """
    return Path.home() / ".cache" / "pi" / "pdf-out"
...
def maybe_spill_to_file(content: str, ext: str, max_stdout_bytes: int) -> str:
    """If content > threshold, spill to ~/.cache/pi/pdf-out/<hash>.<ext>
...
def add_common_args(p: argparse.ArgumentParser) -> None:
    ...
    p.add_argument(
        ...
        help=(
            ...
            f"~/.cache/pi/pdf-out/<hash>.{{md,json}} and only print a "
