...
5. **Edit content**: Update text in each `slide{N}.xml`.
   **Use separate worker sessions here if available** — slides are separate XML files, so worker sessions can edit in parallel via the `team` tool.
...
**Worker sessions:** If available, use separate worker sessions here (after completing step 4). Each slide is a separate XML file, so worker sessions can edit in parallel via the `team` tool. In your prompt to each worker session, include:
