...
if [ "$STATUS" = "READY" ]; then
  ...
  if [ "$LOCALE_HEALED" -eq 1 ]; then
    ...
    echo "  To make UTF-8 sticky for the pi runtime and its worker subprocesses:"
    if [ "$OS" = "macos" ]; then
      echo "    launchctl setenv LANG en_US.UTF-8"
      echo "    launchctl setenv LC_ALL en_US.UTF-8"
      echo "    # then restart the pi session so it inherits the new env"
    elif [ "$OS" = "linux" ] || [ "$OS" = "wsl" ]; then
      echo "    # pi launched via systemd: add to your unit file's [Service] section:"
      echo "    Environment=LANG=en_US.UTF-8"
      echo "    Environment=LC_ALL=en_US.UTF-8"
      echo "    # pi launched from shell: export the two vars in the parent shell"
      echo "    # before starting pi (NOT in ~/.zshrc — see below)."
    fi
    echo "  Note: editing ~/.zshrc does NOT help — pi worker subprocesses are non-interactive"
  ...
  if [ "$LOCALE_BROKEN" -eq 1 ]; then
    ...
    if [ "$OS" = "macos" ]; then
      echo "  macOS (persist for GUI apps + pi session; survives reboot once set in launchd):"
      echo "    launchctl setenv LANG en_US.UTF-8"
      echo "    launchctl setenv LC_ALL en_US.UTF-8"
      echo "  Then restart the pi session so it inherits the new env."
    elif [ "$OS" = "linux" ] || [ "$OS" = "wsl" ]; then
