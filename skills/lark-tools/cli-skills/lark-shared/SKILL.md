...
> **pi 适配说明 (重要)**:
> - pi 没有 mavis 的 daemon。所有 lark-cli 初始化、app-registration、auth 流程均由**用户**或**agent**直接跑 lark-cli 命令完成；不存在 `/mavis/api/lark/onboard/*` HTTP 端点。
> - **绑定新飞书 app（首次使用、新机器）**：用户在终端跑 `lark-cli config init`，按提示输入 appId + appSecret（来自飞书开放平台 https://open.feishu.cn/app）。`config init` 完成后 app 凭据就写入了全局 lark-cli 存储 `~/.lark-cli/`。详见入口 `lark-tools/SKILL.md`。
> - **首次用户身份授权（一次性拿到推荐 scope 全集）**：用户/agent 跑 `lark-cli auth login --recommend`，CLI 展示授权 URL 给用户，用户在浏览器点同意，一次过拿到飞书自动批准的推荐 scope 全集（覆盖 80%+ 日常 domain）。
> - **运行时检测到缺 scope**：按 §[Agent 代理发起认证](#agent-代理发起认证) 的 canonical 模板执行（`--no-wait --json` 取 URL → 后台 `--device-code` 等用户）。**不要** curl 不存在的 daemon 端点、不要再用 `--domain`。仅在推荐 scope 集合之外的高敏感 scope 才会触发这条路径。
> - 本文件其余部分（身份选择原则、scope 概念、更新检查、安全规则）完全适用，无需特殊适配。

## 配置初始化

pi 中**首次绑定 bot** 由用户在终端跑 `lark-cli config init`（详见 `lark-tools/SKILL.md`）。该命令会写好 `~/.lark-cli/config.json` + appsecret。

> 仅当全局 `~/.lark-cli/` 还没有 app 配置时才需要这一步；已有 appId 时跳过。
...
- 在 pi 中，agent 不会自动跑 `--recommend`——需要用户或 agent 在 `config init` 之后显式跑一次 `lark-cli auth login --recommend`；上面的命令只在运行时缺增量 scope 时才需要。
