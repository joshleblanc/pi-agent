...
- **`--as user`（推荐）**：以当前登录用户身份操作其有权访问的 Base。在 pi 中，agent 不会自动跑 `lark-cli auth login --recommend`——用户或 agent 需在 `config init` 之后显式跑一次；若仍提示缺特定 base scope（如 `base:record:update`），按 `cli-skills/lark-shared/SKILL.md` §Agent 代理发起认证 的 canonical 模板执行增量授权。
