...
- **`--as user`（推荐）**：以当前登录用户的身份访问其邮箱。在 pi 中，agent 不会自动跑 `lark-cli auth login --recommend`——用户或 agent 需在 `config init` 之后显式跑一次；若仍报 `LARK_USER_AUTH_REQUIRED` 或缺特定 mail scope，按 `cli-skills/lark-shared/SKILL.md` §Agent 代理发起认证 的 canonical 模板执行增量授权。
