...
>
> - `<SKILL>` → the minimax-xlsx skill folder (in pi: `~/.pi/agent/skills/minimax-xlsx/`)
> - `<SKILL_PDF>` → the minimax-pdf skill folder (in pi: `~/.pi/agent/skills/minimax-pdf/`)
...
## X2 — Multi-format deliverable (Excel + PDF + Word) via the `team` tool
...
STEP 1 — Pre-flight: confirm dotnet for minimax-docx
  ...
  # Either pre-install before launching workers, OR the team plan worker
  # for docx must explicitly budget setup time per STEP 2.

STEP 2 — Build a `team` plan with explicit setup budget
  # CRITICAL: Word worker prompt must include:
  ...
  #    Only consider the worker stalled if no progress for >10 minutes."

  YAML structure:
  - id: dashboard
    assigned_to: <xlsx-capable agent, e.g. general or coder>
    prompt: <follow X1 / X3 / X5 canonical query for the data shape>
    timeout_ms: 1800000

  - id: report-pdf
    assigned_to: <pdf-capable agent>
    prompt: <follow [minimax-pdf] §P3 canonical for chart-rich PDF>
    timeout_ms: 1800000

  - id: memo-docx
    assigned_to: <docx-capable agent>
  ...
  - id: cross-check
    ...
    prompt: "Spot-check 3 numbers that appear in all three deliverables.
             They MUST be identical. If any differ, FAIL the cycle and
             ship a CycleReport identifying the divergent figure."

STEP 3 — Orchestrator stall detection
  # Do NOT treat "no message in 60s" as a stall during the dotnet phase.
  # Only intervene if the worker has been silent for >10 minutes OR
  # the worker explicitly reports a blocker via scratchpad.
...
STEP 5 — Cross-deliverable consistency report
  ...
  summary. Mismatch → which one is wrong? Fix that worker's input data
...
CONSTRAINTS:
  ...
  - Worker prompts: never `2>/dev/null` any setup or build step.
```
