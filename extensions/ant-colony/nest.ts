...
export class Nest {
  ...
  private pheromoneBatch: Pheromone[] = [];
  private batchFlushMs: number = 100;
  private lastBatchFlush: number = 0;
  private pendingTaskWrites: Task[] = [];
  private taskWriteBatchSize: number = 5;
  ...
  /** Flush any pending batched task writes */
  flushTaskWrites(): void {
    for (const task of this.pendingTaskWrites) {
  ...
  writeTask(task: Task): void {
    ...
    this.pendingTaskWrites.push(task);
    // Batch writes for better I/O performance
    if (this.pendingTaskWrites.length >= this.taskWriteBatchSize) {
      this.flushTaskWrites();
    }
  }
  ...
  claimNextTask(caste: "scout" | "worker" | "soldier" | "drone", antId: string): Task | null {
    return this.withStateLock(() => {
      ...
      // Only load pheromones if we have tasks with files
      if (tasks.some(t => t.files.length > 0)) {
      ...
      if (tasks.length > 1 && Math.random() < 0.1) {
        ...
        const scored = tasks.map(t => {
          let pScore = 0;
          // Skip pheromone scoring for tasks without files
          if (t.files.length > 0 && this.pheromoneByFile.size > 0) {
  ...
  /** Flush any pending batched pheromones to disk */
  flushPheromoneBatch(): void {
    if (this.pheromoneBatch.length === 0) return;
    const content = this.pheromoneBatch.map(p => JSON.stringify(p)).join("\n") + "\n";
    fs.appendFileSync(this.pheromoneFile, content);
    this.pheromoneCache.push(...this.pheromoneBatch);
    this.pheromoneBatch = [];
    this.pheromoneIndexDirty = true;
  }

  dropPheromone(p: Pheromone): void {
    // Batch pheromones for better I/O performance
    this.pheromoneBatch.push(p);
    const now = Date.now();
    // Flush if batch is large or enough time has passed
    if (this.pheromoneBatch.length >= 10 || now - this.lastBatchFlush > this.batchFlushMs) {
      this.flushPheromoneBatch();
      this.lastBatchFlush = now;
    } else {
      // Still mark index dirty even if not flushed yet
      this.pheromoneIndexDirty = true;
    }
  }
  ...
  getAllPheromones(): Pheromone[] {
    ...
    const now = Date.now();
    // Pre-calculate decay constant: 0.5^(elapsed/HALF_LIFE) > 0.05
    // This means elapsed < HALF_LIFE * log2(20) ≈ HALF_LIFE * 4.32
    const MAX_AGE = HALF_LIFE * 4.32;
    ...
    // 衰减 + 过滤弱信息素 - optimized with pre-calculated max age
    const beforeLen = this.pheromoneCache.length;
    this.pheromoneCache = this.pheromoneCache.filter(p => {
      const age = now - p.createdAt;
      // Quick filter: if too old, skip decay calculation
      if (age > MAX_AGE) return false;
      p.strength = Math.pow(0.5, age / HALF_LIFE);
  ...
  /** 读取与特定文件相关的信息素摘要 - optimized with early exit */
  getPheromoneContext(files: string[], limit = 20): string {
    const pheromones = this.getAllPheromones();
    
    // Early exit for empty pheromones
    if (pheromones.length === 0) return "";
    
    // Early exit for global context (empty files list)
    if (files.length === 0) {
      const sorted = pheromones
    ...
    const relevant = pheromones
      .filter(p => p.files.some(f => files.includes(f)))
      .sort((a, b) => b.strength - a.strength)
      .slice(0, limit);
  ...
  /** Flush pending pheromones and cleanup resources */
  destroy(): void {
    // Flush any pending batched data before cleanup
    this.flushPheromoneBatch();
    this.flushTaskWrites();
  ...
  private withStateLock<T>(fn: () => T): T {
    ...
    while (true) {
      try {
        ...
        // Windows: use smaller jitter for faster lock acquisition
        // Unix: use original jitter approach
        if (isWindows) {
          // On Windows, use smaller jitter for faster lock acquisition
          const jitter = 1 + Math.random() * 3;
          const until = Date.now() + jitter;
          while (Date.now() < until) { /* minimal spin */ }
        } else {
          // On Unix, the original jitter approach is fine
