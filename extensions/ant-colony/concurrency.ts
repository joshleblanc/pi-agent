...
/** 采样当前系统负载 - optimized for performance */
export function sampleSystem(activeTasks: number, completedRecently: number, windowMinutes: number): ConcurrencySample {
  // Use os.loadavg() which is more efficient than iterating over all CPUs
  const loadavg = os.loadavg();
  // Normalize 1-minute load average to 0-1 range (divide by CPU count, cap at 1)
  const cpuLoad = Math.min(1, loadavg[0] / CPU_CORES);
