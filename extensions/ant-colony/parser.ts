...
// Pre-compiled regex patterns for pheromone extraction
const PHEROMONE_SECTIONS = [
  { name: "Discoveries", type: "discovery" as PheromoneType },
  { name: "Pheromone", type: "progress" as PheromoneType },
  { name: "Files Changed", type: "completion" as PheromoneType },
  { name: "Warnings", type: "warning" as PheromoneType },
  { name: "Review", type: "warning" as PheromoneType },
];
const PHEROMONE_REGEX_MAP = new Map<string, RegExp>();
for (const section of PHEROMONE_SECTIONS) {
  PHEROMONE_REGEX_MAP.set(section.name, new RegExp(`#{1,2} ${section.name}\\n([\\s\\S]*?)(?=\\n#{1,2} |$)`, "i"));
}
...
export function extractPheromones(antId: string, caste: AntCaste, taskId: string, output: string, files: string[], failed = false): Pheromone[] {
  ...
  const now = Date.now();
  for (const section of PHEROMONE_SECTIONS) {
    const regex = PHEROMONE_REGEX_MAP.get(section.name)!;
    const match = output.match(regex);
    if (match?.[1]?.trim()) {
      const type: PheromoneType = section.type;
      pheromones.push({
        id: makePheromoneId(),
        type,
        antId,
